...
public abstract class Entity {
	 ...
	 *            The other ship
	 * 
	 * @return Null if the time until the collision is positive infinity. |if
	 *         (this.getTimeToCollision(otherShip) == POSITIVE_INFINITY) |result
	 *         = null
	 ...
	 *         the y-coordinates on the same moment. Omega, the angle between
	 *         the vertical delta y and the connection between the two centers
	 *         is used to project the distance between the collision point and
	 *         the center on the x- and y-axis. This distance is the radius of
	 *         the ship where the method is invoked on. The sum of these
	 *         projections with their respective positions (x and y of the
	 *         array) is the collision position. |pos_collide1 = {position_1[0]
	 *         + velocity_1[0]*time_till_overlapping, position_1[1] + |
	 *         velocity_1[1]*time_till_overlapping} |pos_collide2 =
	 *         {position_2[0] + velocity_2[0]*time_till_overlapping, |
	 *         position_2[1] + velocity_2[1]*time_till_overlapping}; |delta_x =
	 *         (pos_collide2[0] - pos_collide1[0]); |delta_y = (pos_collide2[1]-
	 *         pos_collide1[1]); |if (delta_x > 0) | omega =
	 *         Math.atan(delta_y/delta_x); |else | omega =
	 *         Math.atan(delta_y/delta_x) + Math.PI; |position_collide =
	 *         {pos_collide1[0] + radius_1*Math.cos(omega), pos_collide1[1] +
	 *         |radius_1*Math.sin(omega)}; |result = position_collide;
	 * 
	 * @throws ModelException
	 *             If the two ships are overlapping. |(this.overlap(otherShip))
	 */
	 ...
	 *         same ship, the distance between is 0. |if (this == otherShip)
	 *         |return 0
	 ...
	 *         from the distance between the centers. |first_pos =
	 *         this.getShipPosition(); |second_pos =
	 *         otherShip.getShipPosition(); |result =
	 *         Math.sqrt(Math.pow(first_pos[0], second_pos[0]) +
	 ...
	 *       will be 0 (they collide). |this.move(getTimeToCollision(Ship
	 *       otherShip)) |otherShip.move(getTimeToCollision(Ship otherShip))
	 ...
	 * 
	 * @throws ModelException
	 *             If the two ships overlap. |(this.overlap(otherShip))
	 */
	...
	// This method will place the entity at any given position, even when this
	// position is an illegal position.
	public void setPositionWhenColliding(double positionX, double positionY) {
		this.position.setPositionX(positionX);
		this.position.setPositionY(positionY);
	}
	...
	public boolean entityFitsInWorld(World world) {
		if (!this.entityLaysInBoundaries(world) || this.entityOverlappingInWorld(world) != null)
			return false;
		
		return true;
	}

	public boolean entityLaysInBoundaries(World world) {
		double radius = this.getEntityRadius();
